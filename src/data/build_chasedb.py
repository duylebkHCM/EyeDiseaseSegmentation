...
if __name__ == "__main__":
    des_dir = Path('../../data/processed/CHASEDB1/')
    source_dir = Path('../../data/raw/CHASEDB1/')
    ...
    split_propotion = 0.2 
    mask_ext = '_1stHO.png'
    train_img, test_img = train_test_split(list((source_dir).glob('*.jpg')), test_size=split_propotion, shuffle=True, random_state=1999)
    ...
    for i, img in enumerate(tqdm(train_img)):
        ...
        mask = img.name[:-4] + mask_ext
        src_mask = Image.open(source_dir / mask)
