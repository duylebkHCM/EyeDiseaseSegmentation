...
class AxialAttention_dynamic(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
        ...
        ## Initial values 
        self.f_qr = nn.Parameter(torch.randn((1, )),  requires_grad=True) 
        self.f_kr = nn.Parameter(torch.randn((1, )), requires_grad=True)  
        self.f_sve = nn.Parameter(torch.randn((1, )), requires_grad=True)
        self.f_sv = nn.Parameter(torch.randn((1, )), requires_grad=True)
...
class medt_net(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x1 = self.layer1(x)
        x2 = self.layer2(x1)
        x = F.relu(F.interpolate(self.decoder4(x2) , scale_factor=(2,2), mode ='bilinear', align_corners=False))
        x = torch.add(x, x1)
        x = F.relu(F.interpolate(self.decoder5(x) , scale_factor=(2,2), mode ='bilinear', align_corners=False))
        x_loc = x.clone()
...
def axialunet_wopo(pretrained=False, **kwargs):
...
def gated(pretrained=False, **kwargs):
    model = ResAxialAttentionUNet(AxialBlock_dynamic, [1, 2, 2, 1], s= 0.0625, **kwargs)
    return model
...
if __name__ == '__main__':
    # model = axialunet(img_size=256, num_classes=1)
    # model1 = MedT(img_size=256, num_classes=1)
    # model2 = logo(img_size=256, num_classes=1)
    model3 = gated(img_size=256, num_classes=1)
